...
record Customer1 (int custId, String customerName, String pickupLocation, String dropLocation, int distance, String phone){
    public Customer1 {
        if (custId < 0){
        ...
        if(pickupLocation == null || pickupLocation.isBlank()){
            throw new IllegalArgumentException("Pickup Location can't be null");
        }
        if(dropLocation == null || dropLocation.isBlank()){
            throw new IllegalArgumentException("Drop Location can't be null");
        }
        if(distance < 0){
...
class CustomerService {
    ...
    public boolean isFirstCustomer(Customer1 customer){
    ...
    public double calculateBill(Customer1 customer) {
    ...
    public String printBill(Customer1 customer) {
...
class CabCustomerServiceTester {
    static void main() {
        int customerId = Integer.parseInt(IO.readln("Enter Customer Id: "));
        ...
        Customer1 C = new Customer1(customerId, customerName, pickupLocation, dropLocation, distance, phone);
        CustomerService Cs = new CustomerService();
        Cs.addCustomer(C);
        IO.println("Your customer has been added");
        IO.println(Cs.printBill(C));
    }
}
